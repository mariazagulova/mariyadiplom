...
def generate_neurons():
    ...
    ################################################### used to be iaf###############
   # nest.SetDefaults('hh_psc_exp', hh_neuronparams) #will we need this?
   # nest.SetDefaults('hh_psc_alpha', hh_neuronparams)

    # without dopamine
    for part in parts_no_nora:
       # part[k_model] = 'hh_psc_exp'
       part[k_model] = 'hh_psc_alpha'
    # with dopamine
...
def connect(pre, post, syn_type=GABA, weight_coef=1):
    ...
    nest.Connect(pre[k_IDs],
                 post[k_IDs],
                 conn_spec=conn_dict,
                 syn_spec=types[syn_type][3] if syn_type in (NA_ex, NA_in) else types[syn_type][0])
...
def save(GUI):
    ...
    SAVE_PATH = "/home/tobias/noratest/bigresults/output-{0}/".format(NEURONS)
    if GUI:
        import pylab as pl
