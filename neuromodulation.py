...
if nora_flag:
    ...
    nest.CopyModel('stdp_noradrenaline_synapse', nora_model_ex, NORA_synparams_ex)
    nest.CopyModel('stdp_noradrenaline_synapse', nora_model_in, NORA_synparams_in)

    connect(nts[nts_a1], lc[locus_coeruleus], syn_type=NA_ex, weight_coef=0.005)
    connect(nts[nts_a2], lc[locus_coeruleus], syn_type=NA_ex, weight_coef=0.005)
    connect(nts[nts_a2], lc[locus_coeruleus], syn_type=NA_ex, weight_coef=0.005)
    connect(nts[nts_a2], striatum[striatumR], syn_type=NA_ex, weight_coef=0.005)
    connect(nts[nts_a2], bnst[bed_nucleus_of_the_stria_terminalis], syn_type=NA_ex, weight_coef=0.005)

    connect(lc[locus_coeruleus], pf[prefrontal_cortex], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], motor[motor_cortex], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], vta[vta_a1], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], rn[rn_a1], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], rn[rn_a2], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], ldt[LDT_a1], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], ldt[LDT_a2], syn_type=NA_ex, weight_coef=0.005)
    connect(lc[locus_coeruleus], striatum[striatumR], syn_type=NA_ex, weight_coef=0.005)

    connect(vta[ventral_tegmental_area], lc[lc_D1], syn_type=NA_in, weight_coef=0.005)
    connect(vta[ventral_tegmental_area], lc[lc_D2], syn_type=NA_in, weight_coef=0.005)
    """
